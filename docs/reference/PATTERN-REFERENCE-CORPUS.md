...
| 16 | Scattered Thoughts of a Crafty Mom Bow Back Knit Top | https://www.scatteredthoughtsofacraftymom.com/bow-back-knit-top-free-pattern/ | Close top/tunic package reference: front/back body, curved hem, optional sleeves, facings, back keyhole, ribbon tie, tiled PDF conventions. Knit/child sizing means it is not first-garment truth. | Free PDF and tutorial, copyrighted by the author. Use as reference-only for package conventions and feature gaps; do not copy geometry or images. | `pattern-reference` | `bow-back-knit-top-reference` |
