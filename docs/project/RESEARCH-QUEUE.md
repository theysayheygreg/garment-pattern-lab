...
- Borrow from CAD-to-technical-drawing workflows only as a reference analog: define `PatternPackageModel`, `PatternPackageComposer`, and `SewingPatternSheetProfile` so human-readable SVG/PDF output has sewing-native pattern sheets, useful dimensions, labels, pattern information, scale proof, page/tile layout, and print-readiness checks.

### R2d: Sewing Pattern Package Composition

Goal: turn validated `PatternGraph` into readable pattern sheets, not just exported paths.

Questions:

- Which fields belong in the prototype pattern information block: pattern name, garment family, size/measurement set, units, seam allowance, revision, validation status, date, source/provenance, and limitations?
...
- Which FreeCAD TechDraw concepts can inspire the internal schema without leaking engineering-drawing language into the designer-facing product?
- Which Open CASCADE hidden-line-removal concepts are useful for future 3D reference views, if any?

Outputs:

- `docs/research/cad-to-technical-drawings-2026-05-03.md`
- `docs/reference/PATTERN-STANDARDS-AND-CONVENTIONS.md`
- `docs/project/PATTERN-SHEET-PROFILE.md`
- first `PatternPackageModel` JSON fixture
