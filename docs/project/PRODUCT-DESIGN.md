...
The product direction now has two companion planning docs:

- [Product Pillars](PRODUCT-PILLARS.md) names the high-level product principles that keep Pattern Lab designer-centered instead of CAD-centered.
- [Prototype Build Order](PROTOTYPE-BUILD-ORDER.md) turns those principles into a build sequence, folder boundary, and validation-first prototype path.
...
The screenshot also highlights two pipeline needs: Kew's visible canvas does not include a 3D render yet, and Kiko's vector-editable/layered-tooling instinct is relevant. Pattern Lab will need semantic vector layers, editable traces, and correction handles soon, but as an interpretation bridge between art and pattern, not as a full Illustrator clone. The Pattern Lab addition is the pattern/validation/3D feedback loop after those layers are understood.
