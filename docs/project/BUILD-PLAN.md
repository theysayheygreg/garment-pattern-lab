...
- [ ] **v0.1 implementation phases B–J** per the design spec. Phase B now has SVG passthrough including common SVG primitives, best-effort vector PDF / `.ai` conversion, VTracer-backed raster tracing, synthetic smoke coverage for every recipe, and a fixture readiness report. Real-world fixture hardening remains open.
- [ ] Pattern schema concrete JSON shape (closes during Phase E).
- [ ] Tech stack decision concrete (closes during Phase B with VTracer-backed sketch ingest + Three.js + pdf-lib commitments; Potrace remains research-only unless license isolation is intentional).
